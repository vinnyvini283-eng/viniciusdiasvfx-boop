...
def lembrete_tarefas_vencendo():
    ...
    try:
        ...
        result = (db.table("tarefas")
                    ...
                    .eq("feito", False)
                    .gte("data_limite", str(hoje))
                    .lte("data_limite", str(amanha))
                    .execute())
        if not result.data:
            return
        linhas = ["⚠️ *Tarefas vencendo em breve:*\n"]
        for t in result.data:
            cliente = (t.get("clientes") or {}).get("nome", "?")
            linhas.append(f"• {cliente}: {t['nome']} → {t['data_limite']}")
        send_message(chat_id, "\n".join(linhas))
    except Exception as e:
        logger.error(f"lembrete_tarefas error: {e}")
...
def start_scheduler():
    ...
    _scheduler.add_job(lembrete_tarefas_vencendo, "cron", hour=9, minute=0)
